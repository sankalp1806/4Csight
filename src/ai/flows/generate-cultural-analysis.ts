...
const prompt = ai.definePrompt({
  ...
  Instructions:
  1.  **Analyze Prevailing Cultural Trends:** Identify 4-5 current, major cultural movements and trends relevant to the brand's industry (e.g., Sustainability, Digital-First Lifestyle, Work-Life Balance, Health Consciousness). {{#if location}}Focus on trends relevant to '{{{location}}}'.{{/if}} For each trend, provide:
  ...
  2.  **Identify Core Cultural Values:** Determine 4-5 fundamental values driving consumer behavior in this market (e.g., Authenticity, Innovation, Community, Convenience). {{#if location}}Focus on values relevant to '{{{location}}}'.{{/if}} For each value, provide:
      - A descriptive name and a short explanation.
  ...
  5. **Provide Regional Insights:** Give a cultural alignment score (0-10) for at least 3 major global regions (e.g., North America, Europe, Asia Pacific). If a specific location is provided (e.g., {{{location}}}), ensure it is one of the regions analyzed.
